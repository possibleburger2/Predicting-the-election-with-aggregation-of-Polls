...
Our polling data analysis was conducted using the R programming language [@citeR]. The dataset, sourced from FiveThirtyEight [@fivethirtyeight2024] and encompassing polling data up to October 21, 2024, provides a comprehensive snapshot of public opinion leading into the election. Following the guidelines in @tellingstories, we examined factors affecting voter support percentages, such as poll timing, polling organization characteristics, and regional differences.

To enhance data manipulation, modeling, and visualization, we utilized several R packages. The `tidyverse` package streamlined data wrangling and analysis, improving workflow efficiency [@thereferencecanbewhatever], while the `here` package managed file paths, aiding reproducibility [@citehere]. For data verification, `testthat` was used to confirm that dataset variables were correctly formatted [@citetestthat]. The `knitr` package’s `kable` function allowed for clean table formatting [@citeknitr], and `janitor` supported data cleaning by addressing any dataset quality issues [@citejanitor]. For date manipulation, `lubridate` simplified handling of time variables [@citelubridate], and `arrow` optimized data I/O in parquet format, essential for managing larger datasets [@citearrow]. The `ggplot2` package facilitated data visualization, adding clarity to the analysis [@citeggplot2].

Specialized packages were also used for specific tasks. `modelsummary` enabled efficient reporting of model summaries, streamlining the presentation of regression results [@citemodelsummary], while `RColorBrewer` provided cohesive color palettes to enhance visualization clarity [@citeRColorBrewer]. To improve computational efficiency during model training, we utilized `future` for parallel processing [@citefuture]. For model validation, the `boot` package supported K-Fold cross-validation to assess stability and reliability [@citeboot1;@citeboot2], and `caret` facilitated training and performance evaluation across cross-validation folds [@citecaret]. Our coding structure and file organization were guided by best practices outlined in @tellingstories.
...
bar_plot <- ggplot(analysis_data, aes(x = pollscore)) +
  ...
  scale_x_continuous(breaks = seq(0, 1.5, by = 0.5)) +  # Adds more frequent x-axis labels
  theme_minimal() +
...
bar_plot2 <- ggplot(analysis_data, aes(x = transparency_score)) +
  ...
  scale_x_continuous(breaks = seq(0,10, by = 2)) +  # Adds more frequent x-axis labels
  theme_minimal() +
  theme(legend.position = "none")
...
@fig-reli shows the distribution of Numeric Grade (left) and Pollscore (right) and transparency Score (bottom) across polling institutions. While Numeric grade and Transperency score both indicate higher quality polls with an increase in value, pollscore represents an increase in quality with a decrease in value. Overall most polls seem to be somewhat high quality based off the three graphs.
